 ...
 *
 * Ported unchanged from oceanum-notebook (packages/share-oceanum/src/client.ts). The status
 ...
 *
 * Granting carries a list because the dialog shares with several people at once, while the
 * spec store takes one grant per request. `invalid` carries back anything typed that is not
 * a shareable address, so the caller can name it rather than failing with "invalid email".
 */
...
function isSummary(value: unknown): value is ISpecSummary {
  ...
  return isSpecId(v.id) && typeof v.name === 'string' && typeof v.modified === 'string';
}
...
export class SpecStoreClient {
  ...
  async get(id: string): Promise<ISpecRecord> {
    return this._record(await this._request('GET', this._url(id), { auth: 'optional' }));
  }

  /** Create a record. The server mints the id: use the returned record's. */
  async create(body: ISpecBody): Promise<ISpecRecord> {
    return this._record(
      await this._request('POST', this._base, { auth: 'required', body: pick(body) })
    );
  }
  ...
  async addPermission(id: string, grant: PermissionGrant): Promise<void> {
    await this._request('POST', `${this._url(id)}/permissions`, {
      auth: 'required',
      body: { type: grant.type, entity: grant.entity, permission: grant.permission }
    });
  }
