...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(COMMANDS.OPEN_DATAMESH, async () => {
      ...
      if (!accessToken && !datameshToken) {
        // Only with neither credential. The Datamesh UI takes either one, so a user who
        // has configured a token is not pushed through a sign-in they do not need -- which
        // is what this did before 0.4.0, whenever there was no access token.
