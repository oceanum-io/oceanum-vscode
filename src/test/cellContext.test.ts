...
import {
  ...
  MAX_NOTEBOOK_CELLS,
  type ContextCell,
} from "../notebook/cellContext";

const code = (source: string): ContextCell => ({ kind: "code", source });
const md = (source: string): ContextCell => ({ kind: "markdown", source });
...
describe("formatNotebookCells", () => {
  ...
  it("falls back to code only when markdown would exceed the byte budget", () => {
    ...
    const codeCells = Array.from({ length: 10 }, (_, i) =>
      code(`x${i} = "${"a".repeat(60_000)}"`),
    );
    const prose = Array.from({ length: 10 }, () => md("b".repeat(60_000)));
