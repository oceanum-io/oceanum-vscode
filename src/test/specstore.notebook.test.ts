...
function notebook(outputText = 'hello', metadata: object = {}): INotebookContent {
  return {
    nbformat: 4,
    nbformat_minor: 5,
    metadata: { kernelspec: { name: 'python', display_name: 'Python' }, ...metadata },
    cells: [
...
describe('readLink', () => {
  it('reads a link for the same spec store', () => {
    expect(readLink({ spec_id: ID, specs_url: SPECS, description: 'd' }, SPECS)).toEqual({
      spec_id: ID,
      specs_url: SPECS,
      description: 'd'
    });
  ...
  it('ignores links to another spec store or with a bad id', () => {
    expect(readLink({ spec_id: ID, specs_url: 'https://specs.dev' }, SPECS)).toBeNull();
...
describe('buildSpecBody', () => {
  ...
  it('strips code-cell outputs when the body is over the limit', () => {
    ...
    const code = result.body.spec.cells[1];
    ...
    expect(content.cells[1].outputs).toHaveLength(1);
  });
  ...
  it('measures UTF-8 bytes, not characters', () => {
    ...
    const size = JSON.stringify({ name: 'n', description: null, spec: content }).length;
...
describe('notebookFromRecord', () => {
  const base = { id: ID, name: 'Waves', modified: '2026-09-12T10:00:00', creator: null };
  ...
  it('replaces a stale link and omits an empty description', () => {
    ...
    const content = notebookFromRecord({ ...base, description: null, spec }, SPECS);
    expect(content.metadata.oceanum).toEqual({ spec_id: ID, specs_url: SPECS });
  });

  it('rejects records that are not notebooks', () => {
    for (const spec of [{}, [], null, { nbformat: 4, cells: {} }, { nbformat: 3 }]) {
      expect(() =>
...
describe('names and paths', () => {
  ...
  it('picks a unique path in the Oceanum folder', () => {
    ...
    expect(uniqueNotebookPath('Waves', ['Waves.ipynb'])).toBe('Oceanum/Waves (1).ipynb');
    expect(
      uniqueNotebookPath('Wa/ves', ['Wa-ves.ipynb', 'Wa-ves (1).ipynb', 'Other.ipynb'])
...
describe('shareLink', () => {
  it('works with an absolute or path base URL', () => {
    ...
    expect(
      shareLink('/sub/', 'https://notebook.example.com/sub/lab/index.html?x=1', ID)
    ).toBe(expected);
  });
});
...
describe('withoutTrust', () => {
  ...
  it('removes trusted from every cell and keeps other metadata', () => {
    ...
    expect(cells.every(cell => (cell.metadata.tags as string[])[0] === 'keep')).toBe(
      true
    );
  });
  ...
  it('is applied when saving, so other clients never receive a trust claim', () => {
    ...
    expect(
      result.ok && result.body.spec.cells.some(cell => 'trusted' in cell.metadata)
    ).toBe(false);
  });
});
...
describe('parseShareEmails', () => {
  it('splits on commas, semicolons, spaces and new lines', () => {
    const raw = 'a@example.com, b@example.com;c@example.com d@example.com\ne@example.com';
  ...
  it('lower-cases addresses, because the store matches the claim case-sensitively', () => {
    expect(parseShareEmails('Alice@Example.COM').emails).toEqual(['alice@example.com']);
  });

  it('removes duplicates, keeping the order first written', () => {
    expect(parseShareEmails('b@example.com a@example.com B@example.com').emails).toEqual([
      'b@example.com',
      'a@example.com'
    ]);
  });
  ...
  it('separates the addresses it cannot grant, reported as they were typed', () => {
    const parsed = parseShareEmails('good@example.com, nope, *@example.com, No@Good');
...
describe('reportForShareFailures', () => {
  it('keeps the original error for a failed public share, which names nobody', () => {
    ...
    expect(reportForShareFailures(1, [''], false)).toEqual({ kind: 'original' });
  });

  it('keeps the original error when the session needs signing in again', () => {
    // Anything synthetic is not a SpecStoreError, so the reporter would not offer sign-in.
    expect(reportForShareFailures(3, ['a@example.com', 'b@example.com'], true)).toEqual({
      kind: 'original'
    });
  });

  it('keeps the original error when every grant failed', () => {
    expect(reportForShareFailures(2, ['a@example.com', 'b@example.com'], false)).toEqual({
      kind: 'original'
    });
  });
  ...
  it('never builds a message that names nobody, even with grants left over', () => {
    ...
    expect(reportForShareFailures(2, [''], false)).toEqual({ kind: 'original' });
  });
...
describe('sanitizeName on a desktop file system', () => {
  // Where this differs from oceanum-notebook's copy, which writes to a browser drive and
  // only has path separators to worry about.
  it('replaces the characters Windows refuses in a file name', () => {
    expect(sanitizeName('Tides: NZ <2026> "draft"?*|')).toBe(
      'Tides- NZ -2026- -draft----'
    );
  });

  it('still produces a usable name from one made only of them', () => {
    expect(sanitizeName('..')).toBe('Untitled');
    expect(uniqueNotebookPath('a/b', [])).toBe('Oceanum/a-b.ipynb');
  });
});
