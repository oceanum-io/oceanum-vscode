 ...
 * multi-line and retry-directive cases this stream does not use. The same
 * parser as oceanumlab's, so the two notebook clients read the stream alike.
 *
...
export async function* readSse(
  ...
  try {
    ...
    // or stopped early -- a `return` from a for-await runs this.
    reader.releaseLock();
  }
}
 ...
 * single line ending.
 *
 * The spec allows all three, and our own backend sends LF -- but that is a
 * statement about one hop. Anything between it and the extension may rewrite
 * line endings, and the failure if it does is total rather than partial: the
 * frame separator `\n\n` never matches `\r\n\r\n`, so ZERO events parse and the
 * response reads as incomplete.
 *
